...
void log_compatibility_issue(uint16_t manufacturer_id, const char* component, const char* format, ...) {
    ...
    print_log_msg(LogLevel::Compat, "COMPAT", 
                  "[%s - Mfg: %s (0x%04X)] %s: %s", 
...
const char* log_level_to_str(enum LogLevel lvl) {
    switch (lvl) {
        ...
        case LogLevel::Scan:    return "SCAN";
        case LogLevel::Compat:  return "COMPAT";
        case LogLevel::Mfg:     return "MFG";
        default:                return "UNKNOWN";
    }
}

enum LogLevel str_to_log_level(const char* str) {
    if (strcasecmp(str, "NONE") == 0)     return LogLevel::None;
    ...
    if (strcasecmp(str, "SCAN") == 0)     return LogLevel::Scan;
    if (strcasecmp(str, "COMPAT") == 0)   return LogLevel::Compat;
    if (strcasecmp(str, "MFG") == 0)      return LogLevel::Mfg;
